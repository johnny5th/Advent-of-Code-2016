let instructions = `bot 135 gives low to bot 27 and high to bot 166
...
class Bot {
  ...
  takeVal(valueType) {
    this.values = this.values.sort((a,b)=>parseInt(a, 10) - parseInt(b, 10));
    if(valueType == 'low')
  ...
  setInstruction(lowRecipient, lowRecIndex, highRecipient, highRecIndex) {
    this.instructions = {
      low: {recipient: lowRecipient, index: lowRecIndex},
      high: {recipient: highRecipient, index: highRecIndex}
    };
  }

  runInstructions() {
    if(this.values.length == 2)
      Object.keys(this.instructions).forEach((valueType) => {
        let val = this.takeVal(valueType);

        if(this.instructions[valueType].recipient == 'bot') {
          console.log('Giving bot', this.instructions[valueType].index, valueType, val);
          bots[this.instructions[valueType].index].giveVal(val);
        } else {
          console.log('Giving output', this.instructions[valueType].index, valueType, val);
          outputs[this.instructions[valueType].index] = val;
        }
      });
    else
      return false;
...
instructions.forEach((instruction) => {
  let value_instruction = instruction.match(/value (\d+) goes to bot (\d+)/);
  let bot_instruction = instruction.match(/bot (\d+) gives low to ([bot|output]+) (\d+) and high to ([bot|output]+) (\d+)/);

  if(value_instruction) {
    let botIndex = value_instruction[2];
    let val = value_instruction[1];

    if(typeof bots[botIndex] == 'undefined')
      bots[botIndex] = new Bot();

    bots[botIndex].giveVal(val);

  } else if(bot_instruction) {
    let botIndex = bot_instruction[1];
    let lowRecipient = bot_instruction[2];
    let lowRecIndex = bot_instruction[3];
    let highRecipient = bot_instruction[4];
    let highRecIndex = bot_instruction[5];

    if(typeof bots[botIndex] == 'undefined')
      bots[botIndex] = new Bot();

    bots[botIndex].setInstruction(lowRecipient, lowRecIndex, highRecipient, highRecIndex);
  }
});

let desiredBot = false;

while(bots.length < 1 || bots.filter((bot)=>bot.values.length > 0).length > 0) {

  for(let botIndex in bots) {
    if((bots[botIndex].values[0] == 61 && bots[botIndex].values[1] == 17) ||
    (bots[botIndex].values[0] == 17 && bots[botIndex].values[1] == 61))
      desiredBot += botIndex+"";
    bots[botIndex].runInstructions();
  }
}

console.log(desiredBot);