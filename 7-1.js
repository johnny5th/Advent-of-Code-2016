let ips = `xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
...
ips = ips.split("\n").map((ip) => {
  ...
  for(let i in matches) {
    ...
    if(match == '[')
      ...
      segments[segments.length - 1].bracketed = true;
      continue;
    }
    ...
    for(let j=0; j<matchArray.length; j++) {
      if(j>0 && matchArray[j+2] !== undefined && matchArray[j-1] != matchArray[j] && matchArray[j-1] == matchArray[j+2] && matchArray[j] == matchArray[j+1])
        supportsTLS = true;
    }

    segments.push({segment: match, supportsTLS: supportsTLS, bracketed: false});
  }

  let supportsTLS = false;
  let bracketedSupportsTLS = false;
  for(let i in segments) {
    if(segments[i].supportsTLS) {
      supportsTLS = true;

      if(segments[i].bracketed)
        bracketedSupportsTLS = true;
    }
  }

  if(bracketedSupportsTLS)
    supportsTLS = false;

  return {segments: segments, supportsTLS: supportsTLS};

});

let supportedCount = ips.filter((ip) => {
