...
ips = ips.split("\n").map((ip) => {
  ...
  for(let i in matches) {
    ...
    for(let j=0; j<matchArray.length; j++) {
      if(matchArray[j+2] !== undefined && matchArray[j] == matchArray[j+2])
        abaList.push(matchArray[j] + matchArray[j+1] + matchArray[j+2]);
    }

    segments.push({segment: match, abaList: abaList, bracketed: false});
  }
  ...
  segments.forEach((segment) => {
    if(segment.bracketed) {
      abaList.forEach((aba) => {
        ...
        var re = new RegExp(bab);
        if(segment.segment.match(re)) {
          supportsSSL = true;
        }
      });
    }
  ...
  return {segments: segments, supportsSSL: supportsSSL};
