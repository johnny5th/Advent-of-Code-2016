let rooms = `aczupnetwp-dnlgpyrpc-sfye-dstaatyr-561[patyc]
...
function roomStats(roomString, checksum, sectorID) {
  let chars = roomString.split('');
  let charMap = new Map();
  ...
  for(let i in chars) {
    ...
    charMap.set(chars[i], charMap.get(chars[i]) ? charMap.get(chars[i])+1 : 1);
  }

  let sortedArray = [...charMap].sort((a, b) => {
  ...
  for(let i in checksumArray) {

    if(checksumArray[i] != sortedArray[i][0])
      valid = false;
  }

  return {valid: valid, sectorID: sectorID, roomString: roomString};
}

function translate(room) {
  function convert(char) {
    if(char == '-')
      return ' ';
    else
      return String.fromCharCode(((char.charCodeAt()-97+ parseInt(room.sectorID, 10)) % 26) + 97);
  }

  let roomStringArray = room.roomString;

  room.translatedID = '';
  for(let i in roomStringArray) {
    room.translatedID += convert(roomStringArray[i]);
  }
...
rooms = rooms.map((room) => {
  let roomValues = room.match(/(.*)-(\d*)\[([a-z]*)\]/);

  let stats = roomStats(roomValues[1], roomValues[3], roomValues[2]);

  return translate(stats);
});
let i = 0;
rooms.forEach((room) => {
  if(room.valid && room.translatedID.match(/pole/))
    console.log(room.translatedID, room.sectorID);
  i++;
});
